...
export const IndonesiaNews = createAsyncThunk(
  ...
  async () => {
    const response = await axios.get(
      `https://newsapi.org/v2/top-headlines?${indonesia}&from=2023-01-01&sortBy=popularity&${apiKey}&${pageSize}`
    );
...
export const ProgrammingNews = createAsyncThunk(
  ...
  async () => {
    const response = await axios.get(
      `https://newsapi.org/v2/everything?${programming}&from=2023-01-01&sortBy=popularity&${apiKey}&${pageSize}`
    );
    return response.data;
  }
);
...
export const CovidNews = createAsyncThunk("articles/fetchNews", async () => {
  const response = await axios.get(
    `https://newsapi.org/v2/everything?${covid}&from=2023-01-01&sortBy=popularity&${apiKey}&${pageSize}`
  );
  return response.data;
});
